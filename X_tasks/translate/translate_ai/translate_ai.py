...
2. **Named Entities:** Translate named entities as follows:
    ...
    * This applies to: organization names, personal names, place names, book titles, movie titles, Chinese idioms (成语典故), proverbs, allegorical phrases (歇后语), and any other Cultural phrases and proper nouns.
...
def is_already_translated(file_path, target_lang):
    """Checks if file is already translated."""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        if target_lang == "en":
            english_chars = sum(1 for char in content.lower() if 'a' <= char <= 'z')
            if english_chars / len(content) > 0.6 if len(content) > 0 else 0:
